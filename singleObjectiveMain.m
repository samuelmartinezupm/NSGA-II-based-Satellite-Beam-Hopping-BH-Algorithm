function [UC,EC,TTS,UC_db,EC_db,TTS_db]=singleObjectiveMain(instance, scenario, maxFEs, popSize, initFun, c_rate, m_type, m_rate, ls_type, ls_rate, run, users)
    addpath('./LEO')
    ...
    UC=min(vars);
    EC=0; %COMPLETAR, VALOR f(2)=EC asociado a esa posición de objectives.
    TTS=0; % COMPLETAR, VALOR f(3)=TTS asociado a esa posición de objectives.
    mop_values = zeros(popSize,3);
    for i=1:popSize
        [~,f,~] = ObjectiveFunction(vars(i,:));
        mop_values(i,:) =  f;
        %mop_values(i,) =  f(2);
        %mop_values(i,) =  f(3);
    end
    ...
    save(strcat('[', instance,'_', num2str(scenario), ...
                ...
                '.r',num2str(run),'.mat'),"vars","objectives", "constraints","vars_db","mop_values","UC_db","EC_db","TTS_db","c_scenario","B_T","b_slots", "P_T", "Adj_c", "Adj_u", "UpC", "n_users", "beams", "theta", "colours", "frame", "frame_dur", "freq");
